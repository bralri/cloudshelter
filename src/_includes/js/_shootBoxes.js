...
export function throwBoxes(
  ...
  const amountOfBoxes = generateAmountOfBoxes();
  ...
  for (let i = 0; i < amountOfBoxes; i++) {
    const boxBody = new CANNON.Body({ mass: 5, shape: boxShape, material: boxMaterial });
    const boxMesh = new THREE.Mesh(boxGeometry, new THREE.MeshBasicMaterial({ color: new THREE.Color() }));
    world.addBody(boxBody);
    scene.add(boxMesh);
    const spreadFactor = Math.random() * 3;
    const randomSpread = new THREE.Vector3(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5).multiplyScalar(spreadFactor);
...
function generateAmountOfBoxes() {
  let newAmount = Math.floor(Math.random() * 4) + 1;
  while (newAmount === previousAmount) {
    newAmount = Math.floor(Math.random() * 4) + 1;
  }
